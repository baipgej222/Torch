...
public class ByteArrays {
	 ...
	 *         <code>array.length</code>/φ) entries whose first
	 ...
	 *
	 * <P>
	 * If possible (i.e., <code>from</code> is 0) this method uses a backward
	 ...
	 * and M. Douglas McIlroy, “Engineering a Sort Function”, <i>Software:
	 * Practice and Experience</i>, 23(11), pages 1249−1265, 1993.
	 *
	 * <p>
	 ...
	 *         otherwise, <samp>(-(<i>insertion point</i>) - 1)</samp>. The
	 ...
	 * McIlroy, Keith Bostic and M. Douglas McIlroy, “Engineering radix sort”,
	 * <i>Computing Systems</i>, 6(1), pages 5−27 (1993), and further improved
	 * using the digit-oracle idea described by Juha Kärkkäinen and Tommi
	 * Rantala in “Engineering radix sort for strings”, <i>String Processing and
	 * Information Retrieval, 15th International Symposium</i>, volume 5280 of
	...
	public static void radixSort(final byte[] a, final int from, final int to) {
		...
		while (offsetPos > 0) {
			...
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This
																									// is
																									// the
																									// shift
																									// that
	 ...
	 * mergesort (stable) already at small sizes (say, more than 10000
	 * elements), but it can only sort in ascending order. It will allocate a
	 * support array of bytes with the same number of elements as the array to
	 * be sorted, and, in the stable case, a further support array as large as
	 * <code>perm</code> (note that the stable version is slightly faster).
	 * 
	 * @param perm
	 ...
	 * <code>a[ i ] == a[ i + 1 ]</code> and <code>b[ i ] <= b[ i + 1 ]</code>.
	 *
	 * <p>
	 * This implementation is significantly faster than quicksort already at
	 * small sizes (say, more than 10000 elements), but it can only sort in
	...
	public static void radixSort(final byte[] a, final byte[] b, final int from, final int to) {
		...
		while (offsetPos > 0) {
			...
			final byte[] k = level < DIGITS_PER_ELEMENT ? a : b; // This is the
																	// key array
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This
																									// is
																									// the
	...
	public static void radixSort(final byte[][] a, final int from, final int to) {
		...
		while (offsetPos > 0) {
			...
			final byte[] k = a[level / DIGITS_PER_ELEMENT]; // This is the key
															// array
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This
																									// is
																									// the
