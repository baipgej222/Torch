...
public final class JavaPluginLoader implements PluginLoader {
	...
	private void removeClass(String name) {
		...
		try {
			...
			// Boggle!
			// (Native methods throwing NPEs is not fun when you can't stop it
			// before-hand)
		}
	}
	...
	public Map<Class<? extends Event>, Set<RegisteredListener>> createRegisteredListeners(Listener listener,
		...
		for (final Method method : methods) {
			...
			EventExecutor executor = new co.aikar.timings.TimedEventExecutor(EventExecutor.create(method, eventClass),
					plugin, method, eventClass); // Spigot // Paper - Use
													// factory method
	...
	public void disablePlugin(Plugin plugin) {
		...
		if (plugin.isEnabled()) {
			...
			if (cloader instanceof PluginClassLoader) {
				...
				names.parallelStream().forEach(name -> { removeClass(name); });
			}
		}
	}
}
