...
public class EntitySlime extends EntityInsentient implements IMonster {
    ...
    public void m() {
        ...
        if (this.onGround && !this.bv) {
            ...
            for (int j = 0; j < i * 8; ++j) {
                ...
                double d0 = this.locXAtom.get() + (double) f2;
                double d1 = this.locZAtom.get() + (double) f3;
    ...
    public void die() {
        ...
        if (!this.world.isClientSide && i > 1 && this.getHealth() <= 0.0F) {
            ...
            for (int k = 0; k < j; ++k) {
                ...
                entityslime.setPositionRotation(this.locXAtom.get() + (double) f, this.locYAtom.get() + 0.5D, this.locZAtom.get() + (double) f1, this.random.nextFloat() * 360.0F, 0.0F);
    ...
    public boolean cG() {
        BlockPosition blockposition = new BlockPosition(MathHelper.floor(this.locXAtom), 0, MathHelper.floor(this.locZAtom));
        ...
        if (this.world.getWorldData().getType() == WorldType.FLAT && this.random.nextInt(4) != 1) {
            ...
            if (this.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
                ...
                if (biomebase == Biomes.h && this.locYAtom.get() > 50.0D && this.locYAtom.get() < 70.0D && this.random.nextFloat() < 0.5F && this.random.nextFloat() < this.world.E() && this.world.getLightLevel(new BlockPosition(this)) <= this.random.nextInt(8)) {
                    return super.cG();
                }

                boolean isSlimeChunk = world.paperConfig.allChunksAreSlimeChunks || chunk.a(987234911L).nextInt(10) == 0; // Paper
                if (this.random.nextInt(10) == 0 && isSlimeChunk && this.locYAtom.get() < 40.0D) { // Paper
                    return super.cG();
                }
            }
    ...
    protected void ci() {
        this.motYAtom.set(0.41999998688697815D);
