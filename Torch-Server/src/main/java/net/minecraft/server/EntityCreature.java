...
public abstract class EntityCreature extends EntityInsentient {
    ...
    public boolean cG() {
        return super.cG() && this.a(new BlockPosition(this.locXAtom.get(), this.getBoundingBox().b, this.locZAtom.get())) >= 0.0F;
    }
    ...
    protected void cP() {
        ...
        if (this.isLeashed() && this.getLeashHolder() != null && this.getLeashHolder().world == this.world) {
            ...
            this.a(new BlockPosition((int) entity.locXAtom.get(), (int) entity.locYAtom.get(), (int) entity.locZAtom.get()), 5);
            ...
            if (f > 6.0F) {
                double d0 = (entity.locXAtom.get() - this.locXAtom.get()) / (double) f;
                double d1 = (entity.locYAtom.get() - this.locYAtom.get()) / (double) f;
                double d2 = (entity.locZAtom.get() - this.locZAtom.get()) / (double) f;

                this.motXAtom.addAndGet(d0 * Math.abs(d0) * 0.4D);
                this.motYAtom.addAndGet(d1 * Math.abs(d1) * 0.4D);
                this.motZAtom.addAndGet(d2 * Math.abs(d2) * 0.4D);
            }
