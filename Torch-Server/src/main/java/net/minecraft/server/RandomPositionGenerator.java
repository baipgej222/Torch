...
public class RandomPositionGenerator {
    ...
    public static Vec3D a(EntityCreature entitycreature, int i, int j, Vec3D vec3d) {
        RandomPositionGenerator.a = vec3d.a(entitycreature.locXAtom.get(), entitycreature.locYAtom.get(), entitycreature.locZAtom.get());
    ...
    public static Vec3D b(EntityCreature entitycreature, int i, int j, Vec3D vec3d) {
        RandomPositionGenerator.a = (new Vec3D(entitycreature.locXAtom.get(), entitycreature.locYAtom.get(), entitycreature.locZAtom.get())).d(vec3d);
        return c(entitycreature, i, j, RandomPositionGenerator.a);
    }
    ...
    private static Vec3D c(EntityCreature entitycreature, int i, int j, @Nullable Vec3D vec3d) {
        ...
        if (entitycreature.cZ()) {
            double d0 = entitycreature.cW().distanceSquared((double) MathHelper.floor(entitycreature.locXAtom), (double) MathHelper.floor(entitycreature.locYAtom), (double) MathHelper.floor(entitycreature.locZAtom)) + 4.0D;
        ...
        for (int j1 = 0; j1 < 10; ++j1) {
            ...
            if (vec3d == null || (double) k1 * vec3d.x + (double) i2 * vec3d.z >= 0.0D) {
                ...
                if (entitycreature.cZ() && i > 1) {
                    ...
                    if (entitycreature.locXAtom.get() > (double) blockposition.getX()) {
                        k1 -= random.nextInt(i / 2);
                    } else {
                        k1 += random.nextInt(i / 2);
                    }

                    if (entitycreature.locZAtom.get() > (double) blockposition.getZ()) {
                ...
                k1 += MathHelper.floor(entitycreature.locXAtom);
                l1 += MathHelper.floor(entitycreature.locYAtom);
                i2 += MathHelper.floor(entitycreature.locZAtom);
