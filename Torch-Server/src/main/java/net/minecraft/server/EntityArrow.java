...
public abstract class EntityArrow extends Entity implements IProjectile {
    ...
    public EntityArrow(World world, EntityLiving entityliving) {
        this(world, entityliving.locXAtom.get(), entityliving.locYAtom.get() + (double) entityliving.getHeadHeight() - 0.10000000149011612D, entityliving.locZAtom.get());
    ...
    public void a(Entity entity, float f, float f1, float f2, float f3, float f4) {
        ...
        // Torch start - use archery attributes
        ...
        this.motXAtom.set(this.motXAtom.get() * velocityTransfer);
        this.motZAtom.set(this.motZAtom.get() * velocityTransfer);
        if (!entity.onGround) {
        	this.motYAtom.set(this.motYAtom.get() * velocityTransfer);
        }
    	// Torch end
    ...
    public void m() {
        ...
        if (this.lastPitch == 0.0F && this.lastYaw == 0.0F) {
            ...
            this.lastYaw = this.yaw = (float) (MathHelper.b(this.motXAtom.get(), this.motZAtom.get()) * 57.2957763671875D);
            this.lastPitch = this.pitch = (float) (MathHelper.b(this.motYAtom.get(), (double) f) * 57.2957763671875D);
        }
        ...
        if (iblockdata.getMaterial() != Material.AIR) {
            ...
            if (axisalignedbb != Block.k && axisalignedbb.a(blockposition).a(new Vec3D(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get()))) {
                this.inGround = true;
            }
        }
        ...
        if (this.inGround) {
            ...
            if (block == this.av && i == this.aw) {
                ...
                this.motYAtom.set(this.motYAtom.get() * (double) (this.random.nextFloat() * 0.2F));
                this.motZAtom.set(this.motZAtom.get() * (double) (this.random.nextFloat() * 0.2F));
                this.ax = 0;
                this.ay = 0;
            }
            ...
            if (this.isCritical()) {
                for (int j = 0; j < 4; ++j) {
                    this.world.addParticle(EnumParticle.CRIT, this.locXAtom.get() + this.motXAtom.get() * (double) j / 4.0D, this.locYAtom.get() + this.motYAtom.get() * (double) j / 4.0D, this.locZAtom.get() + this.motZAtom.get() * (double) j / 4.0D, -this.motXAtom.get(), -this.motYAtom.get() + 0.2D, -this.motZAtom.get(), new int[0]);
                }
            }
            ...
            float f1 = MathHelper.sqrt(this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get());

            this.yaw = (float) (MathHelper.b(this.motXAtom.get(), this.motZAtom.get()) * 57.2957763671875D);

            for (this.pitch = (float) (MathHelper.b(this.motYAtom.get(), (double) f1) * 57.2957763671875D); this.pitch - this.lastPitch < -180.0F; this.lastPitch -= 360.0F) {
                ;
            }
            ...
            if (this.isInWater()) {
                for (int k = 0; k < 4; ++k) {
                    ...
                    this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locXAtom.get() - this.motXAtom.get() * (double) f4, this.locYAtom.get() - this.motYAtom.get() * (double) f4, this.locZAtom.get() - this.motZAtom.get() * (double) f4, this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get(), new int[0]);
                }
            ...
            this.motYAtom.set(this.motYAtom.get() - (double) f3);
    ...
    protected void a(MovingObjectPosition movingobjectposition) {
        ...
        if (entity != null) {
            float f = MathHelper.sqrt(this.motXAtom.get() * this.motXAtom.get() + this.motYAtom.get() * this.motYAtom.get() + this.motZAtom.get() * this.motZAtom.get());
            ...
            if (entity.damageEntity(damagesource, (float) i)) {
                if (entity instanceof EntityLiving) {
                    ...
                    if (this.knockbackStrength > 0) {
                        ...
                        if (f1 > 0.0F) {
                            entityliving.g(this.motXAtom.get() * (double) this.knockbackStrength * 0.6000000238418579D / (double) f1, 0.1D, this.motZAtom.get() * (double) this.knockbackStrength * 0.6000000238418579D / (double) f1);
                        }
                    }
                ...
                this.motXAtom.set(this.motXAtom.get() * -0.10000000149011612D);
                this.motYAtom.set(this.motYAtom.get() * -0.10000000149011612D);
                this.motZAtom.set(this.motZAtom.get() * -0.10000000149011612D);
                this.yaw += 180.0F;
                this.lastYaw += 180.0F;
                this.ay = 0;
                if (!this.world.isClientSide && this.motXAtom.get() * this.motXAtom.get() + this.motYAtom.get() * this.motYAtom.get() + this.motZAtom.get() * this.motZAtom.get() < 0.0010000000474974513D) {
            ...
            this.motXAtom.set((double) (float) (movingobjectposition.pos.x - this.locXAtom.get()));
            this.motYAtom.set((double) (float) (movingobjectposition.pos.y - this.locYAtom.get()));
            this.motZAtom.set((double) (float) (movingobjectposition.pos.z - this.locZAtom.get()));
            float f2 = MathHelper.sqrt(this.motXAtom.get() * this.motXAtom.get() + this.motYAtom.get() * this.motYAtom.get() + this.motZAtom.get() * this.motZAtom.get());

            this.motXAtom.set(this.motXAtom.get() - this.motXAtom.get() / (double) f2 * 0.05000000074505806D);
            this.motYAtom.set(this.motYAtom.get() - this.motYAtom.get() / (double) f2 * 0.05000000074505806D);
            this.motZAtom.set(this.motZAtom.get() - this.motZAtom.get() / (double) f2 * 0.05000000074505806D);
            this.a(SoundEffects.t, 1.0F, 1.2F / (this.random.nextFloat() * 0.2F + 0.9F));
            this.inGround = true;
            this.shake = 7;
    ...
    protected Entity a(Vec3D vec3d, Vec3D vec3d1) {
        Entity entity = null;
        List list = this.world.getEntities(this, this.getBoundingBox().a(this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get()).g(1.0D), EntityArrow.f);
        double d0 = 0.0D;
    ...
    public void d(EntityHuman entityhuman) {
        if (!this.world.isClientSide && this.inGround && this.shake <= 0) {
            ...
            if (this.fromPlayer == PickupStatus.ALLOWED && entityhuman.inventory.canHold(itemstack) > 0) {
                EntityItem item = new EntityItem(this.world, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), itemstack);
