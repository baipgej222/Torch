...
public class ChunkProviderFlat implements ChunkGenerator {
    ...
    /* // Torch start - flat generator
	class ChunkGenerateTask extends RecursiveTask<Chunk> {
		int i; int j;
			
		ChunkGenerateTask(int i, int j) {
			this.i = i;
			this.j = j;
		}
		...
		protected Chunk compute() {
			return originalGetOrCreateChunk(i, j);
		}
	}
	
	@Override
	public Chunk getOrCreateChunk(int i, int j) {
		ChunkGenerateTask task = new ChunkGenerateTask(i, j);
		return SpigotWorldConfig.ChunkPool.invoke(task);
	}
	*/
	...
	public Chunk getOrCreateChunk(int i, int j) {
        ...
        this.e.parallelStream().forEach(worldgenbase -> {
