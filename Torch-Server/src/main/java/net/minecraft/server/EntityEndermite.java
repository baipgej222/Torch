...
public class EntityEndermite extends EntityMonster {
    ...
    public void n() {
        ...
        if (this.world.isClientSide) {
            for (int i = 0; i < 2; ++i) {
                this.world.addParticle(EnumParticle.PORTAL, this.locXAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, this.locYAtom.get() + this.random.nextDouble() * (double) this.length, this.locZAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, (this.random.nextDouble() - 0.5D) * 2.0D, -this.random.nextDouble(), (this.random.nextDouble() - 0.5D) * 2.0D, new int[0]);
            }
        } else {
