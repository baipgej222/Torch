    ...
    protected void a(MovingObjectPosition movingobjectposition) {
        ...
        if (hatching) {
            for (int k = 0; k < numHatching; k++) {
                Entity entity = world.getWorld().createEntity(new org.bukkit.Location(world.getWorld(), this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), this.yaw, 0.0F), hatchingType.getEntityClass());
        ...
        for (int j = 0; j < 8; ++j) {
            this.world.addParticle(EnumParticle.ITEM_CRACK, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), ((double) this.random.nextFloat() - 0.5D) * 0.08D, ((double) this.random.nextFloat() - 0.5D) * 0.08D, ((double) this.random.nextFloat() - 0.5D) * 0.08D, new int[] { Item.getId(Items.EGG)});
        }
