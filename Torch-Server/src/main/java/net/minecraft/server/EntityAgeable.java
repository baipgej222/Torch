...
public abstract class EntityAgeable extends EntityCreature {
            ...
            if (!this.world.isClientSide) {
                ...
                if (oclass != null && this.getClass() == oclass) {
                    ...
                    if (entityageable != null) {
                        ...
                        entityageable.setPositionRotation(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), 0.0F, 0.0F); // Torch
    ...
    public void n() {
        ...
        if (this.world.isClientSide || ageLocked) { // CraftBukkit
            if (this.c > 0) {
                if (this.c % 4 == 0) {
                    this.world.addParticle(EnumParticle.VILLAGER_HAPPY, this.locXAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, this.locYAtom.get() + 0.5D + (double) (this.random.nextFloat() * this.length), this.locZAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, 0.0D, 0.0D, 0.0D, new int[0]); // Torch
                }
