...
public class EntityTrackerEntry {
    ...
    public java.util.Map<EntityPlayer, Boolean> trackedPlayerMap = new java.util.concurrent.ConcurrentHashMap<EntityPlayer, Boolean>(); // Torch
    public Set<EntityPlayer> trackedPlayers = Collections.synchronizedSet(trackedPlayerMap.keySet());
    // Paper end
    ...
    public EntityTrackerEntry(Entity entity, int i, int j, int k, boolean flag) {
        ...
        this.xLoc = EntityTracker.a(entity.locXAtom.get());
        this.yLoc = EntityTracker.a(entity.locYAtom.get());
        this.zLoc = EntityTracker.a(entity.locZAtom.get());
    ...
    public void track(List<EntityHuman> list) {
        ...
        if (!this.isMoving || this.tracker.e(this.q, this.r, this.s) > 16.0D) {
            this.q = this.tracker.locXAtom.get();
            this.r = this.tracker.locYAtom.get();
            this.s = this.tracker.locZAtom.get();
            this.isMoving = true;
            this.b = true;
        ...
        if (this.tracker instanceof EntityItemFrame && this.a % 20 == 0) { // Paper
            ...
            if (itemstack != null && itemstack.getItem() instanceof ItemWorldMap) { // Paper - moved back up
                ...
                this.trackedPlayers.parallelStream().forEach(entityhuman -> { // Torch
        ...
        if (this.a % this.g == 0 || this.tracker.impulse.get() || this.tracker.getDataWatcher().a()) {
            int i;

            if (!this.tracker.isPassenger()) {
                this.v.incrementAndGet();
                long j = EntityTracker.a(this.tracker.locXAtom.get());
                long k = EntityTracker.a(this.tracker.locYAtom.get());
                long l = EntityTracker.a(this.tracker.locZAtom.get());
                ...
                if (this.a > 0 || this.tracker instanceof EntityArrow) { // Paper - Moved up
                    ...
                    this.xRot = j1;
                // CraftBukkit end
                    if (k1 >= -32768L && k1 < 32768L && l1 >= -32768L && l1 < 32768L && i2 >= -32768L && i2 < 32768L && this.v.get() <= 400 && !this.x && this.y == this.tracker.onGround) {
                ...
                if (flag2) {
                    double d0 = this.tracker.motXAtom.get() - this.n;
                    double d1 = this.tracker.motYAtom.get() - this.o;
                    double d2 = this.tracker.motZAtom.get() - this.p;
                    double d3 = 0.02D;
                    double d4 = d0 * d0 + d1 * d1 + d2 * d2;

                    if (d4 > 4.0E-4D || d4 > 0.0D && this.tracker.motXAtom.get() == 0.0D && this.tracker.motYAtom.get() == 0.0D && this.tracker.motZAtom.get() == 0.0D) {
                        this.n = this.tracker.motXAtom.get();
                        this.o = this.tracker.motYAtom.get();
                        this.p = this.tracker.motZAtom.get();
                ...
                this.xLoc = EntityTracker.a(this.tracker.locXAtom.get());
                this.yLoc = EntityTracker.a(this.tracker.locYAtom.get());
                this.zLoc = EntityTracker.a(this.tracker.locZAtom.get());
                this.d();
                this.x = true;
            }
            ...
            this.tracker.impulse.set(false);
        }
    ...
    public void broadcast(Packet<?> packet) {
        this.trackedPlayers.parallelStream().forEach(entityplayer -> { // Torch
        	entityplayer.playerConnection.sendPacket(packet);
        });
    }
            ...
            if (this.c(entityplayer)) {
                if (!this.trackedPlayers.contains(entityplayer) && (this.e(entityplayer) || this.tracker.attachedToPlayer)) {
                    ...
                    if (flag && !(packet instanceof PacketPlayOutSpawnEntityLiving)) {
                        entityplayer.playerConnection.sendPacket(new PacketPlayOutEntityVelocity(this.tracker.getId(), this.tracker.motXAtom.get(), this.tracker.motYAtom.get(), this.tracker.motZAtom.get()));
                    }
                    ...
                    if (this.tracker instanceof EntityLiving) {
                        ...
                        entityliving.getEffects().parallelStream().forEach(mobeffect -> {
                        	entityplayer.playerConnection.sendPacket(new PacketPlayOutEntityEffect(this.tracker.getId(), mobeffect));
                        });
                    }
                    
                    // Torch start - inform new viewer of our passengers
    ...
    private boolean isInRangeOfPlayer(EntityPlayer entityplayer) {
        // Paper end
        double d0 = entityplayer.locXAtom.get() - (double) this.xLoc / 4096.0D;
        double d1 = entityplayer.locZAtom.get() - (double) this.zLoc / 4096.0D;
    ...
    private boolean e(EntityPlayer entityplayer) {
        return entityplayer.x().getPlayerChunkMap().a(entityplayer, this.tracker.ab.get(), this.tracker.ad.get()); // Torch
    }

    public void scanPlayers(List<EntityHuman> list) {
        list.parallelStream().forEach(entityplayer -> {
        	this.updatePlayer((EntityPlayer) entityplayer);
        });
    }
    ...
    private Packet<?> e() {
        ...
        if (this.tracker instanceof EntityItem) {
            ...
            if (this.tracker instanceof EntitySpectralArrow) {
                ...
                packetplayoutspawnentity1.a((int) (this.tracker.motXAtom.get() * 8000.0D));
                packetplayoutspawnentity1.b((int) (this.tracker.motYAtom.get() * 8000.0D));
                packetplayoutspawnentity1.c((int) (this.tracker.motZAtom.get() * 8000.0D));
