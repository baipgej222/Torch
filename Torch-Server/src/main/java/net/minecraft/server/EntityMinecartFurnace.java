...
public class EntityMinecartFurnace extends EntityMinecartAbstract {
    ...
    public void m() {
        ...
        if (this.j() && this.random.nextInt(4) == 0) {
            this.world.addParticle(EnumParticle.SMOKE_LARGE, this.locXAtom.get(), this.locYAtom.get() + 0.8D, this.locZAtom.get(), 0.0D, 0.0D, 0.0D, new int[0]);
        }
    ...
    protected void a(BlockPosition blockposition, IBlockData iblockdata) {
        ...
        if (d0 > 1.0E-4D && this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get() > 0.001D) {
            d0 = (double) MathHelper.sqrt(d0);
            this.a /= d0;
            this.b /= d0;
            if (this.a * this.motXAtom.get() + this.b * this.motZAtom.get() < 0.0D) {
                this.a = 0.0D;
                this.b = 0.0D;
            } else {
                double d1 = d0 / this.o();
    ...
    protected void r() {
        ...
        if (d0 > 1.0E-4D) {
            ...
            this.motXAtom.addAndGet(this.a * d1);
            this.motZAtom.addAndGet(this.b * d1);
        } else {
    ...
    public boolean a(EntityHuman entityhuman, @Nullable ItemStack itemstack, EnumHand enumhand) {
        ...
        this.a = this.locXAtom.get() - entityhuman.locXAtom.get();
        this.b = this.locZAtom.get() - entityhuman.locZAtom.get();
        return true;
    }
