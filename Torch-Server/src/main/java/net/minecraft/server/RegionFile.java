...
import org.torch.util.AtomicByteArray;
...
public class RegionFile {
    ...
    private static final AtomicByteArray a = new AtomicByteArray(4096); // Spigot - note: if this ever changes to not be 4096 bytes, update constructor! // PAIL: empty 4k block
    ...
    public RegionFile(File file) {
        ...
        try {
            ...
            if (this.c.length() < 4096L) {
                ...
                this.c.write(RegionFile.a.getArray()); // Spigot
                this.c.write(RegionFile.a.getArray()); // Spigot
                this.g += 8192;
            }
            ...
            this.f = Collections.synchronizedList(Lists.newArrayListWithCapacity(i));
                    ...
                    if (l + i1 > this.f.size()) {
                        ...
                        if (j1 > 4096 * i1) {
                            ...
                            if (b0 == 1) {
                                abyte = new byte[j1 - 1];
                                this.c.read(abyte);
                                return new DataInputStream(new FastBufferedInputStream(new GZIPInputStream(new FastByteArrayInputStream(abyte)))); // Torch
                            } else if (b0 == 2) {
                                abyte = new byte[j1 - 1];
                                this.c.read(abyte);
                                return new DataInputStream(new FastBufferedInputStream(new InflaterInputStream(new FastByteArrayInputStream(abyte)))); // Torch
                            } else {
                                return null;
                            }
                        }
    ...
    public DataOutputStream b(int i, int j) {
        return this.d(i, j) ? null : new DataOutputStream(new FastBufferedOutputStream(new DeflaterOutputStream(new RegionFile.ChunkBuffer(i, j)))); // Torch
    }
        ...
        try {
            ...
            if (i1 != 0 && j1 == k1) {
                ...
                if (i2 >= k1) {
                    ...
                    for (j2 = 0; j2 < k1; ++j2) {
                        this.c.write(RegionFile.a.getArray());
