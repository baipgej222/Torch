...
public class EntityTippedArrow extends EntityArrow {
    ...
    private void b(int i) {
        ...
        if (j != 0 && i > 0) {
            ...
            for (int k = 0; k < i; ++k) {
                this.world.addParticle(EnumParticle.SPELL_MOB, this.locXAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, this.locYAtom.get() + this.random.nextDouble() * (double) this.length, this.locZAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, d0, d1, d2, new int[0]);
            }
