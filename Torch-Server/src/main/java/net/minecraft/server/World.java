...
public abstract class World implements IBlockAccess {
    ...
    public final List<Entity> entityList = Collections.synchronizedList(new org.spigotmc.OptimizedRemoveAllArrayList<Entity>()
    {
        @Override
    ...
    public final List<EntityHuman> players = Collections.synchronizedList(Lists.newArrayList());
    ...
    public void notify(BlockPosition blockposition, IBlockData iblockdata, IBlockData iblockdata1, int i) {
        this.u.parallelStream().forEach(iworldaccess -> {
        	iworldaccess.a(this, blockposition, iblockdata, iblockdata1, i);
        });
    }
    ...
    public void a(@Nullable EntityHuman entityhuman, double d0, double d1, double d2, SoundEffect soundeffect, SoundCategory soundcategory, float f, float f1) {
        this.u.parallelStream().forEach(iworldaccess -> {
        	iworldaccess.a(entityhuman, soundeffect, soundcategory, d0, d1, d2, f, f1);
        });
    }
    ...
    public void a(BlockPosition blockposition, @Nullable SoundEffect soundeffect) {
        this.u.parallelStream().forEach(iworldaccess -> {
        	iworldaccess.a(soundeffect, blockposition);
        });
    }
    ...
    public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
        ...
        // CraftBukkit end
        int i = MathHelper.floor(entity.locXAtom.get() / 16.0D);
        int j = MathHelper.floor(entity.locZAtom.get() / 16.0D);
            ...
            if (radius > 0) {
                ...
                for (Entity e : entities) {
                    if (e instanceof EntityExperienceOrb) {
                        ...
                        if (!loopItem.dead) {
                            xp.value.set(xp.value.get() + loopItem.value.get()); // Torch
                            loopItem.die();
                        }
                    }
                }
    ...
    public void removeEntity(Entity entity) {
        ...
        int i = entity.ab.get(); // Torch
        int j = entity.ad.get(); // Torch

        if (entity.aa.get() && this.isChunkLoaded(i, j, true)) { // Torch
    ...
    public boolean a(WorldBorder worldborder, Entity entity) {
        ...
        return entity.locXAtom.get() > d0 && entity.locXAtom.get() < d2 && entity.locZAtom.get() > d1 && entity.locZAtom.get() < d3;
    }
    ...
    public void tickEntities() {
        ...
        for (Entity e : this.f) {
            ...
            if (e.aa.get() && this.isChunkLoaded(j, k, true)) { // Torch
                this.getChunkAt(j, k).b(e);
            }
        }

        this.f.parallelStream().forEach(e -> {
        	this.c(e);
        });
        // Paper end
        ...
        for (tickPosition = 0; tickPosition < entityList.size(); tickPosition++) {
            ...
            if (!entity.dead && !(entity instanceof EntityPlayer)) {
                try {
                    ...
                    String msg = "Entity threw exception at " + entity.world.getWorld().getName() + ":" + entity.locXAtom + "," + entity.locYAtom + "," + entity.locZAtom;
            ...
            if (entity.dead) {
                j = entity.ab.get(); // Torch
                int l = entity.ad.get(); // Torch

                if (entity.aa.get() && this.isChunkLoaded(j, l, true)) { // Torch
                    this.getChunkAt(j, l).b(entity);
                }

                guardEntityList = false; // Spigot
                this.entityList.remove(entity); // CraftBukkit - Use field for loop variable // Torch
                guardEntityList = true; // Spigot
                this.c(entity);
            }
        ...
        this.entityList.removeAll(null); // Torch
        guardEntityList = false; // Spigot
    ...
    public void entityJoinedWorld(Entity entity, boolean flag) {
        int i = MathHelper.floor(entity.locXAtom);
        int j = MathHelper.floor(entity.locZAtom);
        byte b0 = 32;
        ...
        if (flag && !org.spigotmc.ActivationRange.checkIfActive(entity)) {
            ...
            // CraftBukkit end
            entity.M = entity.locXAtom.get();
            entity.N = entity.locYAtom.get();
            entity.O = entity.locZAtom.get();
            entity.lastYaw = entity.yaw;
            entity.lastPitch = entity.pitch;
            if (flag && entity.aa.get()) { // Torch
                ++entity.ticksLived;
            ...
            if (Double.isNaN(entity.locXAtom.get()) || Double.isInfinite(entity.locXAtom.get())) {
                entity.locXAtom.set(entity.M);
            }

            if (Double.isNaN(entity.locYAtom.get()) || Double.isInfinite(entity.locYAtom.get())) {
                entity.locYAtom.set(entity.N);
            }

            if (Double.isNaN(entity.locZAtom.get()) || Double.isInfinite(entity.locZAtom.get())) {
                entity.locZAtom.set(entity.O);
            }
            ...
            int k = MathHelper.floor(entity.locXAtom.get() / 16.0D);
            int l = Math.min(15, Math.max(0, MathHelper.floor(entity.locYAtom.get() / 16.0D))); // Paper - stay consistent with chunk add/remove behavior
            int i1 = MathHelper.floor(entity.locZAtom.get() / 16.0D);

            if (!entity.aa.get() || entity.ab.get() != k || entity.ac.get() != l || entity.ad.get() != i1) { // Torch
                if (entity.aa.get() && this.isChunkLoaded(entity.ab.get(), entity.ad.get(), true)) { // Torch
                    this.getChunkAt(entity.ab.get(), entity.ad.get()).a(entity, entity.ac.get()); // Torch
                }
    ...
    public boolean a(AxisAlignedBB axisalignedbb, Material material, Entity entity) {
        ...
        if (!this.isAreaLoaded(i, k, i1, j, l, j1, true)) {
            ...
            if (vec3d.b() > 0.0D && entity.be()) {
                ...
                entity.motXAtom.set(entity.motXAtom.get() + (vec3d.x * d1));
                entity.motYAtom.set(entity.motYAtom.get() + (vec3d.y * d1));
                entity.motZAtom.set(entity.motZAtom.get() + (vec3d.z * d1));
            }
    ...
    public <T extends Entity> List<T> a(Class<? extends T> oclass, Predicate<? super T> predicate) {
        ...
        while (iterator.hasNext()) {
            ...
            if (entity.isToBeRemoved()) { continue; } // Torch
    ...
    public EntityHuman findNearbyPlayer(Entity entity, double d0, Predicate<EntityHuman> filter) {
        return this.findNearbyPlayer(entity.locXAtom.get(), entity.locYAtom.get(), entity.locZAtom.get(), d0, false, filter);
    }
    ...
    public EntityHuman findNearbyPlayerNotInCreativeMode(Entity entity, double d0, Predicate<EntityHuman> filter) {
        return this.findNearbyPlayer(entity.locXAtom.get(), entity.locYAtom.get(), entity.locZAtom.get(), d0, true, filter);
    }
    ...
    public EntityHuman a(Entity entity, double d0, double d1) {
        return this.a(entity.locXAtom.get(), entity.locYAtom.get(), entity.locZAtom.get(), d0, d1, (Function) null, (Predicate) null);
    }
    ...
    public EntityHuman a(double d0, double d1, double d2, double d3, double d4, @Nullable Function<EntityHuman, Double> function, @Nullable Predicate<EntityHuman> predicate) {
        ...
        for (int i = 0; i < this.players.size(); ++i) {
            ...
            if (!entityhuman1.abilities.isInvulnerable && entityhuman1.isAlive() && !entityhuman1.isSpectator() && (predicate == null || predicate.apply(entityhuman1))) {
                double d6 = entityhuman1.e(d0, entityhuman1.locYAtom.get(), d2);
                double d7 = d3;
                ...
                if ((d4 < 0.0D || Math.abs(entityhuman1.locYAtom.get() - d1) < d4 * d4) && (d3 < 0.0D || d6 < d7 * d7) && (d5 == -1.0D || d6 < d5)) {
                    d5 = d6;
                    entityhuman = entityhuman1;
                }
            }
