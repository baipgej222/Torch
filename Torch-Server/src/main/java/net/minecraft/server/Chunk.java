...
public class Chunk {
    ...
    private boolean lit;
    private boolean r;
    private AtomicBoolean s = new AtomicBoolean(); // Torch
    private AtomicBoolean t = new AtomicBoolean(); // Torch
    private long lastSaved;
    private AtomicInteger v = new AtomicInteger(); // Torch
    private long w;
    private int x;
    ...
    // Torch start
    private final AtomicIntegerArray itemCounts = new AtomicIntegerArray(16);
    private final AtomicIntegerArray inventoryEntityCounts = new AtomicIntegerArray(16);
    // Torch end
    // Paper end
    ...
    public Chunk(World world, int i, int j) {
        ...
        this.tileEntities = Maps.newConcurrentMap(); // Torch
        this.x = 4096;
    ...
    public void a(Entity entity) {
        ...
        if (i != this.locX || j != this.locZ) {
            ...
            // CraftBukkit end
            entity.torchSyncAtomLoc();
            // Torch end
        }

        int k = MathHelper.floor(entity.locYAtom.get() / 16.0D);
        ...
        if (entity.aa.get() && entity.getChunkX() == this.locX && entity.getChunkY() == k && entity.getChunkZ() == this.locZ) { // Torch
        ...
        // Torch start - atomic modify
        entity.aa.set(true);
        entity.ab.set(this.locX);
        entity.ac.set(k);
        entity.ad.set(this.locZ);
        // Torch end
        this.entitySlices[k].add(entity);
        // Paper start - update count
        if (ignoreAdd) return;
        if (entity instanceof EntityItem) {
            itemCounts.getAndIncrement(k); // Torch
        } else if (entity instanceof IInventory) {
            inventoryEntityCounts.getAndIncrement(k); // Torch
        }
        // Paper end
    ...
    public void b(Entity entity) {
        this.a(entity, entity.ac.get()); // Torch
    }
    ...
    public void a(Entity entity, int i) {
        ...
        if (entity instanceof EntityItem) {
            itemCounts.getAndDecrement(i);
        } else if (entity instanceof IInventory) {
            inventoryEntityCounts.getAndDecrement(i);
        }
        entity.aa.set(false); // You aren't added to chunk anymore // Torch
        // Paper end
    ...
    public void addEntities() {
        ...
        for (int i = 0; i < this.entitySlices.length; ++i) {
            ...
            this.entitySlices[i].parallelStream().forEach(entity -> {
            	entity.at();
            });
    ...
    public void a(@Nullable Entity entity, AxisAlignedBB axisalignedbb, List<Entity> list, Predicate<? super Entity> predicate) {
        ...
        for (int k = i; k <= j; ++k) {
            if (!this.entitySlices[k].isEmpty()) {
                ...
                */
                if (predicate == IEntitySelector.c && inventoryEntityCounts.get(k) <= 0) continue;
    ...
    public <T extends Entity> void a(Class<? extends T> oclass, AxisAlignedBB axisalignedbb, List<T> list, Predicate<? super T> predicate) {
        ...
        // Paper end
        for (int k = i; k <= j; ++k) {
            if (counts != null && counts.get(k) <= 0) continue; // Paper - Don't check a chunk if it doesn't have the type we are looking for
    ...
    public boolean a(boolean flag) {
        if (flag) {
            if (this.t.get() && this.world.getTime() != this.lastSaved || this.s.get()) { // Torch
                return true;
            }
        } else if (this.t.get() && this.world.getTime() >= this.lastSaved + MinecraftServer.getServer().autosavePeriod * 4) { // Spigot - Only save if we've passed 2 auto save intervals without modification // Torch
            return true;
        }
