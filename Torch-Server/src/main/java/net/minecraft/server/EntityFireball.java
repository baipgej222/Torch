...
public abstract class EntityFireball extends Entity {
    ...
    public EntityFireball(World world, EntityLiving entityliving, double d0, double d1, double d2) {
        ...
        this.setSize(1.0F, 1.0F);
        this.setPositionRotation(entityliving.locXAtom.get(), entityliving.locYAtom.get(), entityliving.locZAtom.get(), entityliving.yaw, entityliving.pitch);
        this.setPosition(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get());
        this.motXAtom.set(0.0D); this.motYAtom.set(0.0D); this.motZAtom.set(0.0D);
        ...
        if (!this.world.isClientSide && (this.shooter != null && this.shooter.dead || !this.world.isLoaded(new BlockPosition(this)))) {
            ...
            if (this.isInWater()) {
                for (int i = 0; i < 4; ++i) {
                    ...
                    this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locXAtom.get() - this.motXAtom.get() * (double) f1, this.locYAtom.get() - this.motYAtom.get() * (double) f1, this.locZAtom.get() - this.motZAtom.get() * (double) f1, this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get(), new int[0]);
                }

                f = 0.8F;
            }

            this.motXAtom.addAndGet(this.dirX);
            this.motYAtom.addAndGet(this.dirY);
            this.motZAtom.addAndGet(this.dirZ);
            this.motXAtom.set(this.motXAtom.get() * (double) f);
            this.motYAtom.set(this.motYAtom.get() * (double) f);
            this.motZAtom.set(this.motZAtom.get() * (double) f);
            this.world.addParticle(this.j(), this.locXAtom.get(), this.locYAtom.get() + 0.5D, this.locZAtom.get(), 0.0D, 0.0D, 0.0D, new int[0]);
            this.setPosition(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get());
        }
    }
    ...
    public void b(NBTTagCompound nbttagcompound) {
        ...
        nbttagcompound.set("direction", this.a(new double[] { this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get()}));
    ...
    public void a(NBTTagCompound nbttagcompound) {
        ...
        if (nbttagcompound.hasKeyOfType("direction", 9) && nbttagcompound.getList("direction", 6).size() == 3) {
            ...
            this.motXAtom.set(nbttaglist.e(0));
            this.motYAtom.set(nbttaglist.e(1));
            this.motZAtom.set(nbttaglist.e(2));
        } else {
            this.die();
        }
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        if (this.isInvulnerable(damagesource)) {
            ...
            if (damagesource.getEntity() != null) {
                ...
                if (vec3d != null) {
                    this.motXAtom.set(vec3d.x);
                    this.motYAtom.set(vec3d.y);
                    this.motZAtom.set(vec3d.z);
                    this.dirX = this.motXAtom.get() * 0.1D;
                    this.dirY = this.motYAtom.get() * 0.1D;
                    this.dirZ = this.motZAtom.get() * 0.1D;
                }
