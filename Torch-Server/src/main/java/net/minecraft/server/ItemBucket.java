        ...
        if (this.a == Blocks.AIR) {
            ...
            if (!world.isEmpty(blockposition) && !flag && !flag1) {
                ...
                if (world.worldProvider.l() && this.a == Blocks.FLOWING_WATER) {
                    ...
                    for (int l = 0; l < 8; ++l) {
                        world.addParticle(EnumParticle.SMOKE_LARGE, (double) i + ThreadLocalRandom.current().nextDouble(), (double) j + ThreadLocalRandom.current().nextDouble(), (double) k + ThreadLocalRandom.current().nextDouble(), 0.0D, 0.0D, 0.0D, new int[0]); // Torch
                    }
                } else {
