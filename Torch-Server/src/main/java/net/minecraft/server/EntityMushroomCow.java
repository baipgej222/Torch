...
public class EntityMushroomCow extends EntityCow {
    ...
    public boolean a(EntityHuman entityhuman, EnumHand enumhand, @Nullable ItemStack itemstack) {
        if (itemstack != null && itemstack.getItem() == Items.BOWL && this.getAge() >= 0 && !entityhuman.abilities.canInstantlyBuild) {
            ...
            // CraftBukkit end
            this.die();
            this.world.addParticle(EnumParticle.EXPLOSION_LARGE, this.locXAtom.get(), this.locYAtom.get() + (double) (this.length / 2.0F), this.locZAtom.get(), 0.0D, 0.0D, 0.0D, new int[0]);
            if (!this.world.isClientSide) {
                EntityCow entitycow = new EntityCow(this.world);

                entitycow.setPositionRotation(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), this.yaw, this.pitch);
                ...
                for (int i = 0; i < 5; ++i) {
                    this.world.addEntity(new EntityItem(this.world, this.locXAtom.get(), this.locYAtom.get() + (double) this.length, this.locZAtom.get(), new ItemStack(Blocks.RED_MUSHROOM)));
                }
