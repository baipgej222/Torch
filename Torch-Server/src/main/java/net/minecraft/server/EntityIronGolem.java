...
public class EntityIronGolem extends EntityGolem {
    ...
    public void n() {
        ...
        if (this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get() > 2.500000277905201E-7D && this.random.nextInt(5) == 0) {
            ...
            if (iblockdata.getMaterial() != Material.AIR) {
                this.world.addParticle(EnumParticle.BLOCK_CRACK, this.locXAtom.get() + ((double) this.random.nextFloat() - 0.5D) * (double) this.width, this.getBoundingBox().b + 0.1D, this.locZAtom.get() + ((double) this.random.nextFloat() - 0.5D) * (double) this.width, 4.0D * ((double) this.random.nextFloat() - 0.5D), 0.5D, ((double) this.random.nextFloat() - 0.5D) * 4.0D, new int[] { Block.getCombinedId(iblockdata)});
            }
        }
    ...
    public boolean B(Entity entity) {
        ...
        if (flag) {
            entity.motYAtom.addAndGet(0.4000000059604645D);
