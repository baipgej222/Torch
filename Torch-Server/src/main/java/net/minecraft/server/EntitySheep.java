        ...
        if (itemstack != null && itemstack.getItem() == Items.SHEARS && !this.isSheared() && !this.isBaby()) {
            if (!this.world.isClientSide) {
                ...
                for (int j = 0; j < i; ++j) {
                    ...
                    entityitem.motYAtom.addAndGet((double) (this.random.nextFloat() * 0.05F));
                    entityitem.motXAtom.addAndGet((double) (this.random.nextFloat() - this.random.nextFloat()) * 0.1F);
                    entityitem.motZAtom.addAndGet((double) (this.random.nextFloat() - this.random.nextFloat()) * 0.1F);
                }
            }
