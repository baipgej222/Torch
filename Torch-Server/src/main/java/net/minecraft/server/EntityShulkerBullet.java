...
public class EntityShulkerBullet extends Entity {
    ...
    private void a(@Nullable EnumDirection.EnumAxis enumdirection_enumaxis) {
        ...
        if (this.target == null) {
            ...
            blockposition = new BlockPosition(this.target.locXAtom.get(), this.target.locYAtom.get() + d0, this.target.locZAtom.get()); // Torch
        }
        ...
        if (blockposition.g(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get()) >= 4.0D) { // Torch
            ...
            d1 = this.locXAtom.get() + (double) enumdirection.getAdjacentX(); // Torch
            d2 = this.locYAtom.get() + (double) enumdirection.getAdjacentY(); // Torch
            d3 = this.locZAtom.get() + (double) enumdirection.getAdjacentZ(); // Torch
        }

        this.a(enumdirection);
        double d4 = d1 - this.locXAtom.get(); // Torch
        double d5 = d2 - this.locYAtom.get(); // Torch
        double d6 = d3 - this.locZAtom.get(); // Torch
        ...
        if (!this.world.isClientSide && this.world.getDifficulty() == EnumDifficulty.PEACEFUL) {
            ...
            if (!this.world.isClientSide) {
                ...
                if (this.target != null && this.target.isAlive() && (!(this.target instanceof EntityHuman) || !((EntityHuman) this.target).isSpectator())) {
                    ...
                    this.motXAtom.addAndGet((this.e - this.motXAtom.get()) * 0.2D); // Torch
                    this.motYAtom.addAndGet((this.f - this.motYAtom.get()) * 0.2D); // Torch
                    this.motZAtom.addAndGet((this.g - this.motZAtom.get()) * 0.2D); // Torch
                } else {
                	this.motYAtom.set(this.motYAtom.get() - 0.04D); // Torch
                }
            ...
            this.setPosition(this.locXAtom.get() + this.motXAtom.get(), this.locYAtom.get() + this.motYAtom.get(), this.locZAtom.get() + this.motZAtom.get()); // Torch
            ProjectileHelper.a(this, 0.5F);
            if (this.world.isClientSide) {
                this.world.addParticle(EnumParticle.END_ROD, this.locXAtom.get() - this.motXAtom.get(), this.locYAtom.get() - this.motYAtom.get() + 0.15D, this.locZAtom.get() - this.motZAtom.get(), 0.0D, 0.0D, 0.0D, new int[0]); // Torch
    ...
    protected void a(MovingObjectPosition movingobjectposition) {
        if (movingobjectposition.entity == null) {
            ((WorldServer) this.world).a(EnumParticle.EXPLOSION_LARGE, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), 2, 0.2D, 0.2D, 0.2D, 0.0D, new int[0]); // Torch
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        if (!this.world.isClientSide) {
            ...
            ((WorldServer) this.world).a(EnumParticle.CRIT, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), 15, 0.2D, 0.2D, 0.2D, 0.0D, new int[0]); // Torch
            this.die();
        }
