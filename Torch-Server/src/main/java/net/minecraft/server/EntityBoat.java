...
public class EntityBoat extends Entity {
    ...
    public void m() {
        ...
        if (!list.isEmpty()) {
            ...
            list.parallelStream().forEach(entityboat -> {
                Entity entity = (Entity) entityboat;
    ...
    private void s() {
        if (this.au > 0 && !this.by()) {
            double d0 = this.locXAtom.get() + (this.av - this.locXAtom.get()) / (double) this.au;
            double d1 = this.locYAtom.get() + (this.aw - this.locYAtom.get()) / (double) this.au;
            double d2 = this.locZAtom.get() + (this.ax - this.locZAtom.get()) / (double) this.au;
    ...
    private void w() {
        ...
        if (this.aH == EntityBoat.EnumStatus.IN_AIR && this.aG != EntityBoat.EnumStatus.IN_AIR && this.aG != EntityBoat.EnumStatus.ON_LAND) {
            ...
            this.setPosition(this.locXAtom.get(), (double) (this.k() - this.length) + 0.101D, this.locZAtom.get());
            this.motYAtom.set(0.0D);
            this.aI = 0.0D;
            ...
            this.motXAtom.set(this.motXAtom.get() * (double) this.g);
            this.motZAtom.set(this.motZAtom.get() * (double) this.g);
            this.at *= this.g;
            this.motYAtom.addAndGet(d1);
            if (d2 > 0.0D) {
                double d3 = 0.65D;

                this.motYAtom.addAndGet(d2 * (-d0 / 0.65D));
                double d4 = 0.75D;

                this.motYAtom.set(this.motYAtom.get() * 0.75D);
            }
        }
    ...
    private void x() {
        if (this.isVehicle()) {
            ...
            this.motXAtom.addAndGet((double) (MathHelper.sin(-this.yaw * 0.017453292F) * f));
            this.motXAtom.addAndGet((double) (MathHelper.cos(this.yaw * 0.017453292F) * f));
    ...
    public void k(Entity entity) {
        if (this.w(entity)) {
            ...
            entity.setPosition(this.locXAtom.get() + vec3d.x, this.locYAtom.get() + (double) f1, this.locZAtom.get() + vec3d.z);
            entity.yaw += this.at;
