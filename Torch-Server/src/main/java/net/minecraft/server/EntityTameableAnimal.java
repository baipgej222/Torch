...
public abstract class EntityTameableAnimal extends EntityAnimal implements EntityOwnable {
    ...
    protected void o(boolean flag) {
        ...
        for (int i = 0; i < 7; ++i) {
            ...
            this.world.addParticle(enumparticle, this.locXAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, this.locYAtom.get() + 0.5D + (double) (this.random.nextFloat() * this.length), this.locZAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, d0, d1, d2, new int[0]);
        }
