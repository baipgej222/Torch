...
public abstract class BlockFluids extends Block {
    ...
    protected void fizz(World world, BlockPosition blockposition) {
        ...
        for (int i = 0; i < 8; ++i) {
            world.addParticle(EnumParticle.SMOKE_LARGE, d0 + ThreadLocalRandom.current().nextDouble(), d1 + 1.2D, d2 + ThreadLocalRandom.current().nextDouble(), 0.0D, 0.0D, 0.0D, new int[0]); // Torch
        }
