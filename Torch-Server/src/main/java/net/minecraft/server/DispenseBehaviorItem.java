...
public class DispenseBehaviorItem implements IDispenseBehavior {
    ...
    public static boolean a(World world, ItemStack itemstack, int i, EnumDirection enumdirection, ISourceBlock isourceblock) {
        ...
        entityitem.motXAtom.set((double) enumdirection.getAdjacentX() * d3);
        entityitem.motYAtom.set(0.20000000298023224D);
        entityitem.motZAtom.set((double) enumdirection.getAdjacentZ() * d3);
        entityitem.motXAtom.set(entityitem.motXAtom.get() + world.random.nextGaussian() * 0.007499999832361937D * (double) i);
        entityitem.motYAtom.set(entityitem.motYAtom.get() + world.random.nextGaussian() * 0.007499999832361937D * (double) i);
        entityitem.motZAtom.set(entityitem.motZAtom.get() + world.random.nextGaussian() * 0.007499999832361937D * (double) i);

        // CraftBukkit start
        org.bukkit.block.Block block = world.getWorld().getBlockAt(isourceblock.getBlockPosition().getX(), isourceblock.getBlockPosition().getY(), isourceblock.getBlockPosition().getZ());
        CraftItemStack craftItem = CraftItemStack.asCraftMirror(itemstack);

        BlockDispenseEvent event = new BlockDispenseEvent(block, craftItem.clone(), new org.bukkit.util.Vector(entityitem.motXAtom.get(), entityitem.motYAtom.get(), entityitem.motZAtom.get()));
        ...
        entityitem.motXAtom.set(event.getVelocity().getX());
        entityitem.motYAtom.set(event.getVelocity().getY());
        entityitem.motZAtom.set(event.getVelocity().getZ());
