    ...
    public void m() {
        ...
        if (!isSilent && this.lifeTicks == 2) { // Spigot
            ...
            for (EntityPlayer player : (List<EntityPlayer>) (List) this.world.players) {
                ...
                if (distanceSquared > viewDistance * viewDistance) {
                    ...
                    player.playerConnection.sendPacket(new PacketPlayOutNamedSoundEffect(SoundEffects.di, SoundCategory.WEATHER, relativeX, this.locYAtom.get(), relativeZ, 10000.0F, pitch));
                } else {
                    player.playerConnection.sendPacket(new PacketPlayOutNamedSoundEffect(SoundEffects.di, SoundCategory.WEATHER, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), 10000.0F, pitch));
                }
            }
            // CraftBukkit end
            this.world.a((EntityHuman) null, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), SoundEffects.dh, SoundCategory.WEATHER, 2.0F, 0.5F + this.random.nextFloat() * 0.2F);
        }
        ...
        if (this.lifeTicks >= 0 && !this.isEffect) { // CraftBukkit - add !this.isEffect
            if (this.world.isClientSide) {
                ...
                double d0 = 3.0D;
                List list = this.world.getEntities(this, new AxisAlignedBB(this.locXAtom.get() - d0, this.locYAtom.get() - d0, this.locZAtom.get() - d0, this.locXAtom.get() + d0, this.locYAtom.get() + 6.0D + d0, this.locZAtom.get() + d0));

                list.parallelStream().forEach(entity -> {
                	((Entity) entity).onLightningStrike(this);
                });
            }
        }
