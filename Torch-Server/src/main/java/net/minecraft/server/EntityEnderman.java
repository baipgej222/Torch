...
public class EntityEnderman extends EntityMonster {
    ...
    public void o() {
        if (this.ticksLived >= this.by + 400) {
            ...
            if (!this.ad()) {
                this.world.a(this.locXAtom.get(), this.locYAtom.get() + (double) this.getHeadHeight(), this.locZAtom.get(), SoundEffects.aZ, this.bA(), 2.5F, 1.0F, false);
            }
        }
    ...
    private boolean c(EntityHuman entityhuman) {
        ...
        if (itemstack != null && itemstack.getItem() == Item.getItemOf(Blocks.PUMPKIN)) {
            ...
            Vec3D vec3d1 = new Vec3D(this.locXAtom.get() - entityhuman.locXAtom.get(), this.getBoundingBox().b + (double) this.getHeadHeight() - (entityhuman.locYAtom.get() + (double) entityhuman.getHeadHeight()), this.locZAtom.get() - entityhuman.locZAtom.get());
            double d0 = vec3d1.b();
    ...
    public void n() {
        if (this.world.isClientSide) {
            for (int i = 0; i < 2; ++i) {
                this.world.addParticle(EnumParticle.PORTAL, this.locXAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, this.locYAtom.get() + this.random.nextDouble() * (double) this.length - 0.25D, this.locZAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, (this.random.nextDouble() - 0.5D) * 2.0D, -this.random.nextDouble(), (this.random.nextDouble() - 0.5D) * 2.0D, new int[0]);
            }
        }
    ...
    protected boolean db() {
        double d0 = this.locXAtom.get() + (this.random.nextDouble() - 0.5D) * 64.0D;
        double d1 = this.locYAtom.get() + (double) (this.random.nextInt(64) - 32);
        double d2 = this.locZAtom.get() + (this.random.nextDouble() - 0.5D) * 64.0D;

        return this.l(d0, d1, d2);
    }

    protected boolean a(Entity entity) {
        Vec3D vec3d = new Vec3D(this.locXAtom.get() - entity.locXAtom.get(), this.getBoundingBox().b + (double) (this.length / 2.0F) - entity.locYAtom.get() + (double) entity.getHeadHeight(), this.locZAtom.get() - entity.locZAtom.get());

        vec3d = vec3d.a();
        double d0 = 16.0D;
        double d1 = this.locXAtom.get() + (this.random.nextDouble() - 0.5D) * 8.0D - vec3d.x * d0;
        double d2 = this.locYAtom.get() + (double) (this.random.nextInt(16) - 8) - vec3d.y * d0;
        double d3 = this.locZAtom.get() + (this.random.nextDouble() - 0.5D) * 8.0D - vec3d.z * d0;
    ...
    private boolean l(double d0, double d1, double d2) {
        ...
        if (flag) {
            this.world.a((EntityHuman) null, this.lastXAtom.get(), this.lastYAtom.get(), this.lastZAtom.get(), SoundEffects.ba, this.bA(), 1.0F, 1.0F);
    ...
    static class PathfinderGoalEndermanPickupBlock extends PathfinderGoal {
        ...
        public void e() {
            ...
            int i = MathHelper.floor(this.enderman.locXAtom.get() - 2.0D + random.nextDouble() * 4.0D);
            int j = MathHelper.floor(this.enderman.locYAtom.get() + random.nextDouble() * 3.0D);
            int k = MathHelper.floor(this.enderman.locZAtom.get() - 2.0D + random.nextDouble() * 4.0D);
            BlockPosition blockposition = new BlockPosition(i, j, k);
            IBlockData iblockdata = world.getType(blockposition);
            Block block = iblockdata.getBlock();
            MovingObjectPosition movingobjectposition = world.rayTrace(new Vec3D((double) ((float) MathHelper.floor(this.enderman.locXAtom) + 0.5F), (double) ((float) j + 0.5F), (double) ((float) MathHelper.floor(this.enderman.locZAtom) + 0.5F)), new Vec3D((double) ((float) i + 0.5F), (double) ((float) j + 0.5F), (double) ((float) k + 0.5F)), false, true, false);
    ...
    static class PathfinderGoalEndermanPlaceBlock extends PathfinderGoal {
        ...
        public void e() {
            ...
            int i = MathHelper.floor(this.a.locXAtom.get() - 1.0D + random.nextDouble() * 2.0D);
            int j = MathHelper.floor(this.a.locYAtom.get() + random.nextDouble() * 2.0D);
            int k = MathHelper.floor(this.a.locZAtom.get() - 1.0D + random.nextDouble() * 2.0D);
            BlockPosition blockposition = new BlockPosition(i, j, k);
            IBlockData iblockdata = world.getType(blockposition);
    ...
    static class PathfinderGoalPlayerWhoLookedAtTarget extends PathfinderGoalNearestAttackableTarget<EntityHuman> {
        ...
        public boolean a() {
            ...
            this.j = this.i.world.a(this.i.locXAtom.get(), this.i.locYAtom.get(), this.i.locZAtom.get(), d0, d0, (Function) null, new Predicate() {
