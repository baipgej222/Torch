...
public class EntityMinecartTNT extends EntityMinecartAbstract {
    ...
    public void m() {
        ...
        if (this.a > 0) {
            --this.a;
            this.world.addParticle(EnumParticle.SMOKE_NORMAL, this.locXAtom.get(), this.locYAtom.get() + 0.5D, this.locZAtom.get(), 0.0D, 0.0D, 0.0D, new int[0]);
        } else if (this.a == 0) {
            this.c(this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get());
        }

        if (this.positionChanged) {
            double d0 = this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get();
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        ...
        if (entity instanceof EntityArrow) {
            ...
            if (entityarrow.isBurning()) {
                this.c(entityarrow.motXAtom.get() * entityarrow.motXAtom.get() + entityarrow.motYAtom.get() * entityarrow.motYAtom.get() + entityarrow.motZAtom.get() * entityarrow.motZAtom.get());
            }
        }
    ...
    protected void c(double d0) {
        if (!this.world.isClientSide) {
            ...
            this.world.explode(this, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), (float) (4.0D + this.random.nextDouble() * 1.5D * d1), true);
            this.die();
        }
    ...
    public void j() {
        ...
        if (!this.world.isClientSide) {
            ...
            if (!this.ad()) {
                this.world.a((EntityHuman) null, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), SoundEffects.gk, SoundCategory.BLOCKS, 1.0F, 1.0F);
            }
        }
