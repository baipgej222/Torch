...
import org.torch.TerrainGeneratingTask;
...
public abstract class MinecraftServer implements Runnable, ICommandListener, IAsyncTaskHandler, IMojangStatistics {
    ...
    private final Random r = new LightRandom();
    ...
    // Torch start - Queue network progress
    ...
    protected void l() {
		...
		for (WorldServer worldserver : worlds) { // Torch
            ...
            // Torch start - Improve terrian generating
			List<int[]> regions = Lists.newLinkedList();
            ...
            for (int k = -radius; k <= radius && this.isRunning(); k += 16) { // Paper
                for (int l = -radius; l <= radius && this.isRunning(); l += 16) { // Paper
                    ...
                    ++i;
                    // worldserver.getChunkProviderServer().getChunkAt(blockposition.getX() + k >> 4, blockposition.getZ() + l >> 4); // Torch
                    positions[0] = blockposition.getX() + k >> 4;
            		positions[1] = blockposition.getZ() + l >> 4;
        			regions.add(positions);
        			// Torch end
                }
            }
            ForkJoinPool.commonPool().invoke(new TerrainGeneratingTask(worldserver, regions)); // Torch
        }
        
        worlds.parallelStream().forEach(world -> {
        	this.server.getPluginManager().callEvent(new org.bukkit.event.world.WorldLoadEvent(world.getWorld())); // Torch
        });
        // CraftBukkit end
        this.t();
    }
    ...
    protected void C() throws ExceptionWorldConflict { // CraftBukkit - added throws
        ...
        if (autosavePeriod > 0 && this.ticks % autosavePeriod == 0) { // CraftBukkit
            ...
            worlds.parallelStream().forEach(world -> { world.getWorld().save(false); }); // Torch
    ...
    public void D() {
        ...
        if (this.ticks % 20 == 0) {
            this.getPlayerList().players.parallelStream().forEach(entityplayer -> {
        ...
        for (ITickable it : this.o) { ((ITickable) it).c(); }
    ...
    public void a(EnumDifficulty enumdifficulty) {
        this.worlds.parallelStream().forEach(worldserver -> {
            if (worldserver != null) {
    ...
    public void setGamemode(WorldSettings.EnumGamemode worldsettings_enumgamemode) {
        this.worlds.parallelStream().forEach(world -> {
