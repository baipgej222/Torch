...
public class EntityEnderDragon extends EntityInsentient implements IComplex, IMonster {
    ...
    public void n() {
        ...
        if (this.world.isClientSide) {
            ...
            if (!this.ad()) {
                ...
                if (f1 <= -0.3F && f >= -0.3F) {
                    this.world.a(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), SoundEffects.aQ, this.bA(), 5.0F, 0.8F + this.random.nextFloat() * 0.3F, false);
                }

                if (!this.bK.a().a() && --this.bL < 0) {
                    this.world.a(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), SoundEffects.aR, this.bA(), 2.5F, 0.8F + this.random.nextFloat() * 0.3F, false);
        ...
        if (this.getHealth() <= 0.0F) {
            ...
            this.world.addParticle(EnumParticle.EXPLOSION_LARGE, this.locXAtom.get() + (double) f, this.locYAtom.get() + 2.0D + (double) f1, this.locZAtom.get() + (double) f2, 0.0D, 0.0D, 0.0D, new int[0]);
        } else {
            this.cW();
            f = 0.2F / (MathHelper.sqrt(this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get()) * 10.0F + 1.0F);
            f *= (float) Math.pow(2.0D, this.motYAtom.get());
            if (this.bK.a().a()) {
                this.bE += 0.1F;
            } else if (this.bF) {
                this.bE += f * 0.5F;
            ...
            if (this.hasAI()) {
                ...
                this.b[this.c][1] = this.locYAtom.get();
                double d0;
                double d1;
                double d2;
                float f3;
                ...
                if (this.world.isClientSide) {
                    if (this.bh.get() > 0) {
                        double d3 = this.locXAtom.get() + (this.bi - this.locXAtom.get()) / (double) this.bh.get();

                        d0 = this.locYAtom.get() + (this.bj - this.locYAtom.get()) / (double) this.bh.get();
                        d1 = this.locZAtom.get() + (this.bk - this.locZAtom.get()) / (double) this.bh.get();
                        d2 = MathHelper.g(this.bl - (double) this.yaw);
                        this.yaw = (float) ((double) this.yaw + d2 / (double) this.bh.get());
                    ...
                    if (vec3d != null && idragoncontroller.getControllerPhase() != DragonControllerPhase.k) { // CraftBukkit - Don't move when hovering // PAIL: rename
                        d0 = vec3d.x - this.locXAtom.get();
                        d1 = vec3d.y - this.locYAtom.get();
                        d2 = vec3d.z - this.locZAtom.get();
                        double d4 = d0 * d0 + d1 * d1 + d2 * d2;

                        f3 = idragoncontroller.f();
                        d1 = MathHelper.a(d1 / (double) MathHelper.sqrt(d0 * d0 + d2 * d2), (double) (-f3), (double) f3);
                        this.motYAtom.addAndGet(d1 * 0.10000000149011612D);
                        this.yaw = MathHelper.g(this.yaw);
                        double d5 = MathHelper.a(MathHelper.g(180.0D - MathHelper.b(d0, d2) * 57.2957763671875D - (double) this.yaw), -50.0D, 50.0D);
                        Vec3D vec3d1 = (new Vec3D(vec3d.x - this.locXAtom.get(), vec3d.y - this.locYAtom.get(), vec3d.z - this.locZAtom.get())).a();
                        Vec3D vec3d2 = (new Vec3D((double) MathHelper.sin(this.yaw * 0.017453292F), this.motYAtom.get(), (double) (-MathHelper.cos(this.yaw * 0.017453292F)))).a();
                        ...
                        if (this.bF) {
                            this.move(this.motXAtom.get() * 0.800000011920929D, this.motYAtom.get() * 0.800000011920929D, this.motZAtom.get() * 0.800000011920929D);
                        } else {
                            this.move(this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get());
                        }

                        Vec3D vec3d3 = (new Vec3D(this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get())).a();
                        float f7 = ((float) vec3d3.b(vec3d2) + 1.0F) / 2.0F;

                        f7 = 0.8F + 0.15F * f7;
                        this.motXAtom.set(this.motXAtom.get() * (double) f7);
                        this.motZAtom.set(this.motZAtom.get() * (double) f7);
                        this.motYAtom.set(this.motYAtom.get() * 0.9100000262260437D);
                    }
                }
                ...
                this.bx.m();
                this.bx.setPositionRotation(this.locXAtom.get() + (double) (f10 * 0.5F), this.locYAtom.get(), this.locZAtom.get() - (double) (f11 * 0.5F), 0.0F, 0.0F);
                this.bB.m();
                this.bB.setPositionRotation(this.locXAtom.get() + (double) (f11 * 4.5F), this.locYAtom.get() + 2.0D, this.locZAtom.get() + (double) (f10 * 4.5F), 0.0F, 0.0F);
                this.bC.m();
                this.bC.setPositionRotation(this.locXAtom.get() - (double) (f11 * 4.5F), this.locYAtom.get() + 2.0D, this.locZAtom.get() - (double) (f10 * 4.5F), 0.0F, 0.0F);
                ...
                this.bv.setPositionRotation(this.locXAtom.get() + (double) (f12 * 6.5F * f2), this.locYAtom.get() + (double) f14 + (double) (f8 * 6.5F), this.locZAtom.get() - (double) (f13 * 6.5F * f2), 0.0F, 0.0F);
                this.bw.setPositionRotation(this.locXAtom.get() + (double) (f12 * 5.5F * f2), this.locYAtom.get() + (double) f14 + (double) (f8 * 5.5F), this.locZAtom.get() - (double) (f13 * 5.5F * f2), 0.0F, 0.0F);
                ...
                for (int j = 0; j < 3; ++j) {
                    ...
                    entitycomplexpart.m();
                    entitycomplexpart.setPositionRotation(this.locXAtom.get() - (double) ((f10 * f17 + f15 * f18) * f2), this.locYAtom.get() + (adouble1[1] - adouble[1]) - (double) ((f18 + f17) * f8) + 1.5D, this.locZAtom.get() + (double) ((f11 * f17 + f16 * f18) * f2), 0.0F, 0.0F);
                }
    ...
    private void a(List<Entity> list) {
        ...
        while (iterator.hasNext()) {
            ...
            if (entity instanceof EntityLiving) {
                double d2 = entity.locXAtom.get() - d0;
                double d3 = entity.locZAtom.get() - d1;
    ...
    private void b(List<Entity> list) {
        list.parallelStream().forEach(entityenderdragon -> {
            Entity entity = (Entity) entityenderdragon;
    ...
    protected void bD() {
        ...
        if (this.bG >= 180 && this.bG <= 200) {
            ...
            this.world.addParticle(EnumParticle.EXPLOSION_HUGE, this.locXAtom.get() + (double) f, this.locYAtom.get() + 2.0D + (double) f1, this.locZAtom.get() + (double) f2, 0.0D, 0.0D, 0.0D, new int[0]);
        }
            ...
            if (this.bG == 1) {
                ...
                for (EntityHuman human : world.players) {
                    ...
                    if (distanceSquared > viewDistance * viewDistance) {
                        ...
                        player.playerConnection.sendPacket(new PacketPlayOutWorldEvent(1028, new BlockPosition((int) relativeX, (int) this.locYAtom.get(), (int) relativeZ), 0, true));
                    } else {
                        player.playerConnection.sendPacket(new PacketPlayOutWorldEvent(1028, new BlockPosition((int) this.locXAtom.get(), (int) this.locYAtom.get(), (int) this.locZAtom.get()), 0, true));
                    }
                }
                // CraftBukkit end
            }
    ...
    public int o() {
        ...
        return this.l(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get());
    }
