...
public class EntityGuardian extends EntityMonster {
    ...
    public void n() {
        if (this.world.isClientSide) {
            ...
            if (!this.isInWater()) {
                this.bx = 2.0F;
                if (this.motYAtom.get() > 0.0D && this.bC && !this.ad()) {
                    this.world.a(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), SoundEffects.ck, this.bA(), 1.0F, 1.0F, false);
                }

                this.bC = this.motYAtom.get() < 0.0D && this.world.d((new BlockPosition(this)).down(), false);
            } else if (this.o()) {
                if (this.bx < 0.5F) {
                    this.bx = 4.0F;
                } else {
            ...
            if (this.o() && this.isInWater()) {
                ...
                for (int i = 0; i < 2; ++i) {
                    this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locXAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width - vec3d.x * 1.5D, this.locYAtom.get() + this.random.nextDouble() * (double) this.length - vec3d.y * 1.5D, this.locZAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width - vec3d.z * 1.5D, 0.0D, 0.0D, 0.0D, new int[0]);
                }
            }
            ...
            if (this.de()) {
                ...
                if (entityliving != null) {
                    ...
                    double d1 = entityliving.locXAtom.get() - this.locXAtom.get();
                    double d2 = entityliving.locYAtom.get() + (double) (entityliving.length * 0.5F) - (this.locYAtom.get() + (double) this.getHeadHeight());
                    double d3 = entityliving.locZAtom.get() - this.locZAtom.get();
                    ...
                    while (d5 < d4) {
                        ...
                        this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locXAtom.get() + d1 * d5, this.locYAtom.get() + d2 * d5 + (double) this.getHeadHeight(), this.locZAtom.get() + d3 * d5, 0.0D, 0.0D, 0.0D, new int[0]);
                    }
                }
            }
        }
        ...
        if (this.inWater) {
            ...
            this.motYAtom.addAndGet(0.5D);
            this.motXAtom.addAndGet((double) ((this.random.nextFloat() * 2.0F - 1.0F) * 0.4F));
            this.motZAtom.addAndGet((double) ((this.random.nextFloat() * 2.0F - 1.0F) * 0.4F));
    ...
    public void g(float f, float f1) {
        if (this.cp()) {
            if (this.isInWater()) {
                ...
                this.motXAtom.set(this.motXAtom.get() * 0.8999999761581421D);
                this.motYAtom.set(this.motYAtom.get() * 0.8999999761581421D);
                this.motZAtom.set(this.motZAtom.get() * 0.8999999761581421D);
                if (!this.o() && this.getGoalTarget() == null) {
                    this.motYAtom.set(this.motYAtom.get() - 0.005D);
                }
            } else {
                super.g(f, f1);
            }
    ...
    static class ControllerMoveGuardian extends ControllerMove {
        ...
        public void c() {
            if (this.h == ControllerMove.Operation.MOVE_TO && !this.i.getNavigation().n()) {
                double d0 = this.b - this.i.locXAtom.get();
                double d1 = this.c - this.i.locYAtom.get();
                double d2 = this.d - this.i.locZAtom.get();
                ...
                this.i.motXAtom.addAndGet(d4 * d5);
                this.i.motZAtom.addAndGet(d4 * d6);
                d4 = Math.sin((double) (this.i.ticksLived + this.i.getId()) * 0.75D) * 0.05D;
                this.i.motYAtom.addAndGet(d4 * (d6 + d5) * 0.25D);
                this.i.motYAtom.addAndGet((double) this.i.cl() * d1 * 0.1D);
                ControllerLook controllerlook = this.i.getControllerLook();
                double d7 = this.i.locXAtom.get() + d0 / d3 * 2.0D;
                double d8 = (double) this.i.getHeadHeight() + this.i.locYAtom.get() + d1 / d3;
                double d9 = this.i.locZAtom.get() + d2 / d3 * 2.0D;
