...
public class EntityZombie extends EntityMonster {
    ...
    public void n() {
        if (this.world.B() && !this.world.isClientSide && !this.isBaby()) {
            float f = this.e(1.0F);
            BlockPosition blockposition = this.bz() instanceof EntityBoat ? (new BlockPosition(this.locXAtom.get(), (double) Math.round(this.locYAtom.get()), this.locZAtom.get())).up() : new BlockPosition(this.locXAtom.get(), (double) Math.round(this.locYAtom.get()), this.locZAtom.get()); // Torch
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        if (super.damageEntity(damagesource, f)) {
            ...
            if (entityliving != null && this.world.getDifficulty() == EnumDifficulty.HARD && (double) this.random.nextFloat() < this.getAttributeInstance(EntityZombie.a).getValue() && this.world.getGameRules().getBoolean("doMobSpawning")) {
                int i = MathHelper.floor(this.locXAtom); // Torch
                int j = MathHelper.floor(this.locYAtom); // Torch
                int k = MathHelper.floor(this.locZAtom); // Torch
    ...
    public void b(EntityLiving entityliving) {
        ...
        if ((this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD) && entityliving instanceof EntityVillager) {
            ...
            this.world.a((EntityHuman) null, 1026, new BlockPosition((int) this.locXAtom.get(), (int) this.locYAtom.get(), (int) this.locZAtom.get()), 0); // Torch
        }
        ...
        if (object instanceof EntityZombie.GroupDataZombie) {
            ...
            if (entityzombie_groupdatazombie.a) {
                ...
                if ((double) this.world.random.nextFloat() < 0.05D) {
                    ...
                    entitychicken1.setPositionRotation(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), this.yaw, 0.0F); // Torch
    ...
    protected void di() {
        ...
        this.world.a((EntityHuman) null, 1027, new BlockPosition((int) this.locXAtom.get(), (int) this.locYAtom.get(), (int) this.locZAtom.get()), 0); // Torch
    }
    ...
    protected int getConversionTime() {
        ...
        if (this.random.nextFloat() < 0.01F) {
            ...
            for (int k = (int) this.locXAtom.get() - 4; k < (int) this.locXAtom.get() + 4 && j < 14; ++k) { // Torch
                for (int l = (int) this.locYAtom.get() - 4; l < (int) this.locYAtom.get() + 4 && j < 14; ++l) { // Torch
                    for (int i1 = (int) this.locZAtom.get() - 4; i1 < (int) this.locZAtom.get() + 4 && j < 14; ++i1) { // Torch
