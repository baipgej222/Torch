...
public class EntityRabbit extends EntityAnimal {
    ...
    protected void ci() {
        ...
        if (d0 > 0.0D) {
            double d1 = this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get();
        ...
        if (this.onGround) {
            ...
            if (this.getRabbitType() == 99 && this.bB == 0) {
                ...
                if (entityliving != null && this.h(entityliving) < 16.0D) {
                    this.a(entityliving.locXAtom.get(), entityliving.locZAtom.get());
                    this.moveController.a(entityliving.locXAtom.get(), entityliving.locYAtom.get(), entityliving.locZAtom.get(), this.moveController.b());
                    this.da();
                    this.bA = true;
                }
            }
    ...
    private void a(double d0, double d1) {
        this.yaw = (float) (MathHelper.b(d1 - this.locZAtom.get(), d0 - this.locXAtom.get()) * 57.2957763671875D) - 90.0F;
    }
    ...
    protected void dd() {
        ...
        this.world.addParticle(EnumParticle.BLOCK_DUST, this.locXAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, this.locYAtom.get() + 0.5D + (double) (this.random.nextFloat() * this.length), this.locZAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, 0.0D, 0.0D, 0.0D, new int[] { Block.getCombinedId(iblockdata)});
        this.bC = 40;
    }
