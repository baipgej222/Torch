        ...
        if (this.e > 0) {
            ...
            if (this.e == 0) {
                ...
                for (int i = 0; i < inventorysubcontainer.getSize(); ++i) {
                    ...
                    if (itemstack1 != null) {
                        double d0 = this.f.locYAtom.get() - 0.30000001192092896D + (double) this.f.getHeadHeight();
                        EntityItem entityitem = new EntityItem(this.f.world, this.f.locXAtom.get(), d0, this.f.locZAtom.get(), itemstack1);
                        float f = 0.3F;
                        float f1 = this.f.aP;
                        float f2 = this.f.pitch;

                        entityitem.motXAtom.set((double) (-MathHelper.sin(f1 * 0.017453292F) * MathHelper.cos(f2 * 0.017453292F) * f));
                        entityitem.motZAtom.set((double) (MathHelper.cos(f1 * 0.017453292F) * MathHelper.cos(f2 * 0.017453292F) * f));
                        entityitem.motYAtom.set((double) (-MathHelper.sin(f2 * 0.017453292F) * f + 0.1F));
                        entityitem.q();
