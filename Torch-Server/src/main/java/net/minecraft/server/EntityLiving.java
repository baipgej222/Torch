...
import net.minecraft.server.Entity.EntityMoveTask;

public abstract class EntityLiving extends Entity {
	
    // Torch start - add archery attributes
    ...
    protected float aZ;
    protected float ba;
    protected AtomicInteger bb = new AtomicInteger();
    public float lastDamage;
    protected boolean bd;
    public float be;
    public float bf;
    public float bg;
    protected AtomicInteger bh = new AtomicInteger();
    protected double bi;
    protected double bj;
    protected double bk;
    protected double bl;
    ...
    public EntityLiving(World world) {
        ...
        this.i = true;
        this.aM = (float) ((ThreadLocalRandom.current().nextDouble() + 1.0D) * 0.009999999776482582D);
    ...
    protected void initAttributes() {
        ...
        // Torch start - init our attributes
        ...
        if (!this.world.isClientSide && this.fallDistance > 3.0F && flag) {
            ...
            if (iblockdata.getMaterial() != Material.AIR) {
                ...
                if (this instanceof EntityPlayer) {
                    ((WorldServer) this.world).sendParticles((EntityPlayer) this, EnumParticle.BLOCK_DUST, false, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), i, 0.0D, 0.0D, 0.0D, 0.15000000596046448D, new int[] { Block.getCombinedId(iblockdata)});
                } else {
                    ((WorldServer) this.world).a(EnumParticle.BLOCK_DUST, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), i, 0.0D, 0.0D, 0.0D, 0.15000000596046448D, new int[] { Block.getCombinedId(iblockdata)});
                }
                // CraftBukkit end
            }
        }
                ...
                if (!this.bC() && !this.hasEffect(MobEffects.WATER_BREATHING) && !flag1) {
                    ...
                    if (this.getAirTicks() == -20) {
                        ...
                        for (int i = 0; i < 8; ++i) {
                            ...
                            this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locXAtom.get() + (double) f, this.locYAtom.get() + (double) f1, this.locZAtom.get() + (double) f2, this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get(), new int[0]);
                        }
    ...
    protected void bD() {
        ...
        if (this.deathTicks >= 20 && !this.dead) { // CraftBukkit - (this.deathTicks == 20) -> (this.deathTicks >= 20 && !this.dead)
            ...
            while (i > 0) {
                ...
                i -= j;
                this.world.addEntity(new EntityExperienceOrb(this.world, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), j));
            }
            this.expToDrop = 0;
            // CraftBukkit end
            ...
            for (i = 0; i < 20; ++i) {
                ...
                this.world.addParticle(EnumParticle.EXPLOSION_NORMAL, this.locXAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, this.locYAtom.get() + (double) (this.random.nextFloat() * this.length), this.locZAtom.get() + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, d0, d1, d2, new int[0]);
            }
        }
    ...
    protected void tickPotionEffects() {
        ...
        if (i > 0) {
            ...
            if (flag1 && i > 0) {
                ...
                this.world.addParticle(flag ? EnumParticle.SPELL_MOB_AMBIENT : EnumParticle.SPELL_MOB, this.locXAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, this.locYAtom.get() + this.random.nextDouble() * (double) this.length, this.locZAtom.get() + (this.random.nextDouble() - 0.5D) * (double) this.width, d0, d1, d2, new int[0]);
            }
        }
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if (false && f > 0.0F && this.d(damagesource)) {
                    ...
                    if (damagesource.a()) {
                        ...
                        if (damagesource.i() instanceof EntityLiving) {
                            ((EntityLiving) damagesource.i()).a(this, 0.5F, this.locXAtom.get() - damagesource.i().locXAtom.get(), this.locZAtom.get() - damagesource.i().locZAtom.get());
                        }
                    }
                ...
                if (flag1) {
                    ...
                    if (entity != null) {
                        ...
                        for (d1 = entity.locZAtom.get() - this.locZAtom.get(); d0 * d0 + d1 * d1 < 1.0E-4D; d1 = (Math.random() - Math.random()) * 0.01D) {
                            d0 = (ThreadLocalRandom.current().nextDouble() - ThreadLocalRandom.current().nextDouble()) * 0.01D;
                        }
    ...
    private boolean d(DamageSource damagesource) {
        if (!damagesource.ignoresArmor() && this.isBlocking()) {
            ...
            if (vec3d != null) {
                ...
                Vec3D vec3d2 = vec3d.a(new Vec3D(this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get())).a();
    ...
    public void b(ItemStack itemstack) {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            vec3d1 = vec3d1.add(this.locXAtom.get(), this.locYAtom.get() + (double) this.getHeadHeight(), this.locZAtom.get());
    ...
    public void die(DamageSource damagesource) {
        if (!this.aU) {
            ...
            if (this.bb.get() >= 0 && entityliving != null) {
                entityliving.b(this, this.bb.get());
            }
    ...
    public void a(Entity entity, float f, double d0, double d1) {
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            this.motXAtom.set(this.motXAtom.get() / 2.0D);
            this.motZAtom.set(this.motZAtom.get() / 2.0D);
            this.motXAtom.set(this.motXAtom.get() - d1 / (double) f1 * (double) f);
            this.motZAtom.set(this.motZAtom.get() - d1 / (double) f1 * (double) f);
            if (this.onGround) {
                this.motYAtom.set(this.motXAtom.get() / 2.0D);
                this.motYAtom.addAndGet((double) f);
                if (this.motYAtom.get() > 0.4000000059604645D) {
                    this.motYAtom.set(0.4000000059604645D);
                }
            }
    ...
    public void e(float f, float f1) {
        ...
        if (i > 0) {
            ...
            int k = MathHelper.floor(this.locYAtom.get() - 0.20000000298023224D);
            int l = MathHelper.floor(this.locZAtom);
       ...
       if (!this.isInvulnerable(damagesource)) {
            ...
            if (event.getDamage(DamageModifier.BLOCKING) < 0) {
                ...
                if (damagesource.i() instanceof EntityLiving) {// PAIL: rename
                    ((EntityLiving) damagesource.i()).a(EntityLiving.this, 0.5F, EntityLiving.this.locXAtom.get() - damagesource.i().locXAtom.get(), EntityLiving.this.locZAtom.get() - damagesource.i().locZAtom.get());
                }
                if (f <= 0) {
                    return false;
                }
    ...
    public void A(Entity entity) {
        ...
        if (!(entity instanceof EntityBoat) && !(entity instanceof EntityHorse)) {
            ...
            double d3 = Math.floor(this.locXAtom.get()) + 0.5D;
            double d4 = Math.floor(this.locZAtom.get()) + 0.5D;
            ...
            for (int j = 0; j < i; ++j) {
                ...
                if (!this.world.b(axisalignedbb1)) {
                    if (this.world.getType(new BlockPosition(d9, this.locYAtom.get(), d10)).q()) {
                        this.enderTeleportTo(d9, this.locYAtom.get() + 1.0D, d10);
                        return;
                    }

                    BlockPosition blockposition = new BlockPosition(d9, this.locYAtom.get() - 1.0D, d10);

                    if (this.world.getType(blockposition).q() || this.world.getType(blockposition).getMaterial() == Material.WATER) {
                        d1 = d9;
                        d2 = this.locYAtom.get() + 1.0D;
                        d0 = d10;
                    }
                } else if (!this.world.b(axisalignedbb1.c(0.0D, 1.0D, 0.0D)) && this.world.getType(new BlockPosition(d9, this.locYAtom.get() + 1.0D, d10)).q()) {
                    d1 = d9;
                    d2 = this.locYAtom.get() + 2.0D;
                    d0 = d10;
                }
            }
            ...
            double d12 = this.locXAtom.get() + (double) f1 * d0;
            double d13 = this.locZAtom.get() + (double) f2 * d0;

            this.setPosition(d12, entity.locYAtom.get() + (double) entity.length + 0.001D, d13);
            if (this.world.b(this.getBoundingBox())) {
                this.setPosition(d12, entity.locYAtom.get() + (double) entity.length + 1.001D, d13);
                if (this.world.b(this.getBoundingBox())) {
                    this.setPosition(entity.locXAtom.get(), entity.locYAtom.get() + (double) this.length + 0.001D, entity.locZAtom.get());
                }
            }
        }
    }
    ...
    protected void ci() {
        this.motYAtom.set((double) this.ch());
        if (this.hasEffect(MobEffects.JUMP)) {
            this.motYAtom.addAndGet((double) ((float) (this.getEffect(MobEffects.JUMP).getAmplifier() + 1) * 0.1F));
        }

        if (this.isSprinting()) {
            float f = this.yaw * 0.017453292F;

            this.motXAtom.set(this.motXAtom.get() - (double) (MathHelper.sin(f) * 0.2F));
            this.motZAtom.addAndGet((double) (MathHelper.cos(f) * 0.2F));
        }
    ...
    protected void cj() {
        this.motYAtom.addAndGet(0.03999999910593033D);
    }
    ...
    public void g(float f, float f1) {
        ...
        if (this.cp() || this.by()) {
            ...
            if (this.isInWater() && (!(this instanceof EntityHuman) || !((EntityHuman) this).abilities.isFlying)) {
                ...
                this.motXAtom.set(this.motXAtom.get() * (double) f4);
                this.motYAtom.set(this.motYAtom.get() * 0.800000011920929D);
                this.motZAtom.set(this.motZAtom.get() * (double) f4);
                this.motYAtom.set(this.motYAtom.get() - 0.02D);
                if (this.positionChanged && this.c(this.motXAtom.get(), this.motYAtom.get() + 0.6000000238418579D - this.locYAtom.get() + d1, this.motZAtom.get())) {
                    this.motYAtom.set(0.30000001192092896D);
                }
                ...
                double d2 = Math.sqrt(this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get());
                double d3 = vec3d.b();
                float f6 = MathHelper.cos(f5);

                f6 = (float) ((double) f6 * (double) f6 * Math.min(1.0D, d3 / 0.4D));
                this.motYAtom.addAndGet(-0.08D + (double) f6 * 0.06D);
                double d4;

                if (this.motYAtom.get() < 0.0D && d0 > 0.0D) {
                    d4 = this.motYAtom.get() * -0.1D * (double) f6;
                    this.motYAtom.addAndGet(d4);
                    this.motXAtom.addAndGet(vec3d.x * d4 / d0);
                    this.motZAtom.addAndGet(vec3d.z * d4 / d0);
                }

                if (f5 < 0.0F) {
                    d4 = d2 * (double) (-MathHelper.sin(f5)) * 0.04D;
                    this.motYAtom.addAndGet(d4 * 3.2D);
                    this.motXAtom.set(this.motXAtom.get() - vec3d.x * d4 / d0);
                    this.motZAtom.set(this.motZAtom.get() - vec3d.x * d4 / d0);
                }

                if (d0 > 0.0D) {
                    this.motXAtom.addAndGet((vec3d.x / d0 * d2 - this.motXAtom.get()) * 0.1D);
                    this.motZAtom.addAndGet((vec3d.z / d0 * d2 - this.motZAtom.get()) * 0.1D);
                }

                this.motXAtom.set(this.motXAtom.get() * 0.9900000095367432D);
                this.motYAtom.set(this.motYAtom.get() * 0.9800000190734863D);
                this.motZAtom.set(this.motZAtom.get() * 0.9900000095367432D);
                this.move(this.motXAtom.get(), this.motYAtom.get(), this.motZAtom.get());
                if (this.positionChanged && !this.world.isClientSide) {
                    d4 = Math.sqrt(this.motXAtom.get() * this.motXAtom.get() + this.motZAtom.get() * this.motZAtom.get());
                    double d5 = d2 - d4;
                ...
                float f8 = 0.91F;
                BlockPosition.PooledBlockPosition blockposition_pooledblockposition = BlockPosition.PooledBlockPosition.d(this.locXAtom.get(), this.getBoundingBox().b - 1.0D, this.locZAtom.get());
                ...
                f8 = 0.91F;
                if (this.onGround) {
                    f8 = this.world.getType(blockposition_pooledblockposition.e(this.locXAtom.get(), this.getBoundingBox().b - 1.0D, this.locZAtom.get())).getBlock().frictionFactor * 0.91F;
                }

                if (this.n_()) {
                    f2 = 0.15F;
                    this.motXAtom.set(MathHelper.a(this.motXAtom.get(), (double) (-f2), (double) f2));
                    this.motZAtom.set(MathHelper.a(this.motZAtom.get(), (double) (-f2), (double) f2));
                    this.fallDistance = 0.0F;
                    if (this.motYAtom.get() < -0.15D) {
                        this.motYAtom.set(-0.15D);
                    }

                    boolean flag = this.isSneaking() && this instanceof EntityHuman;

                    if (flag && this.motYAtom.get() < 0.0D) {
                ...
                if (this.positionChanged && this.n_()) {
                    this.motYAtom.set(0.2D);
                }

                if (this.hasEffect(MobEffects.LEVITATION)) {
                    this.motYAtom.addAndGet((0.05D * (double) (this.getEffect(MobEffects.LEVITATION).getAmplifier() + 1) - this.motYAtom.get()) * 0.2D);
                } else {
                    blockposition_pooledblockposition.e(this.locXAtom.get(), 0.0D, this.locZAtom.get());
                    if (this.world.isClientSide && (!this.world.isLoaded(blockposition_pooledblockposition) || !this.world.getChunkAtWorldCoords(blockposition_pooledblockposition).p())) {
                        if (this.locYAtom.get() > 0.0D) {
                            this.motYAtom.set(-0.1D);
                        } else {
                            this.motYAtom.set(0.0D);
                        }
                    } else {
                        this.motYAtom.set(this.motYAtom.get() - 0.08D);
                    }
                }

                this.motYAtom.set(this.motYAtom.get() * 0.9800000190734863D);
                this.motXAtom.set(this.motXAtom.get() * (double) f8);
                this.motZAtom.set(this.motZAtom.get() * (double) f8);
                blockposition_pooledblockposition.t();
            }
        }

        this.aF = this.aG;
        d1 = this.locXAtom.get() - this.lastXAtom.get();
        d0 = this.locZAtom.get() - this.lastZAtom.get();
    ...
    public void n() {
        ...
        if (this.bh.get() > 0 && !this.by()) {
            double d0 = this.locXAtom.get() + (this.bi - this.locXAtom.get()) / (double) this.bh.get();
            double d1 = this.locYAtom.get() + (this.bj - this.locYAtom.get()) / (double) this.bh.get();
            double d2 = this.locZAtom.get() + (this.bk - this.locZAtom.get()) / (double) this.bh.get();
            double d3 = MathHelper.g(this.bl - (double) this.yaw);

            this.yaw = (float) ((double) this.yaw + d3 / (double) this.bh.get());
            this.pitch = (float) ((double) this.pitch + (this.bm - (double) this.pitch) / (double) this.bh.get());
            this.bh.decrementAndGet();
            this.setPosition(d0, d1, d2);
            this.setYawPitch(this.yaw, this.pitch);
        } else if (!this.cp()) {
            this.motXAtom.set(this.motXAtom.get() * 0.98D);
            this.motYAtom.set(this.motYAtom.get() * 0.98D);
            this.motZAtom.set(this.motZAtom.get() * 0.98D);
        }

        if (Math.abs(this.motXAtom.get()) < 0.003D) {
            this.motXAtom.set(0.0D);
        }

        if (Math.abs(this.motYAtom.get()) < 0.003D) {
            this.motYAtom.set(0.0D);
        }

        if (Math.abs(this.motZAtom.get()) < 0.003D) {
	...
	// Torch start - entity collide task
	class EntityCollideTask extends RecursiveAction {
		EntityCollideTask() {}
	...
	protected void co() {
		EntityCollideTask task = new EntityCollideTask();
		ForkJoinPool.commonPool().execute(task);
	}

	protected void co_task() {
    	// Torch end
        List list = this.world.getEntities(this, this.getBoundingBox(), IEntitySelector.a(this));

        if (!list.isEmpty()) {
                numCollisions.set(Math.max(0, numCollisions.get() - world.spigotConfig.maxCollisionsPerEntity)); // Spigot
                for (int i = 0; i < list.size() && numCollisions.get() < world.spigotConfig.maxCollisionsPerEntity; ++i) {
                ...
                entity.numCollisions.getAndIncrement(); // Spigot
                numCollisions.getAndIncrement(); // Spigot
                this.C(entity);
            }
        }
    ...
    public boolean hasLineOfSight(Entity entity) {
        return this.world.rayTrace(new Vec3D(this.locXAtom.get(), this.locYAtom.get() + (double) this.getHeadHeight(), this.locZAtom.get()), new Vec3D(entity.locXAtom.get(), entity.locYAtom.get() + (double) entity.getHeadHeight(), entity.locZAtom.get()), false, true, false) == null;
    }
    ...
    public boolean k(double d0, double d1, double d2) {
        double d3 = this.locXAtom.get();
        double d4 = this.locYAtom.get();
        double d5 = this.locZAtom.get();
        ...
        if (world.isLoaded(blockposition)) {
            ...
            if (flag1) {
                ...
                EntityTeleportEvent teleport = new EntityTeleportEvent(this.getBukkitEntity(), new Location(this.world.getWorld(), d3, d4, d5), new Location(this.world.getWorld(), this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get()));
        ...
        if (!flag) {
            ...
            for (int i = 0; i < short0; ++i) {
                ...
                double d7 = d3 + (this.locXAtom.get() - d3) * d6 + (random.nextDouble() - 0.5D) * (double) this.width * 2.0D;
                double d8 = d4 + (this.locYAtom.get() - d4) * d6 + random.nextDouble() * (double) this.length;
                double d9 = d5 + (this.locZAtom.get() - d5) * d6 + (random.nextDouble() - 0.5D) * (double) this.width * 2.0D;
