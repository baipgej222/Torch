...
public class EntityCreeper extends EntityMonster {
    ...
    protected boolean a(EntityHuman entityhuman, EnumHand enumhand, @Nullable ItemStack itemstack) {
        if (itemstack != null && itemstack.getItem() == Items.FLINT_AND_STEEL) {
            this.world.a(entityhuman, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), SoundEffects.bx, this.bA(), 1.0F, this.random.nextFloat() * 0.4F + 0.8F);
    ...
    private void dg() {
        if (!this.world.isClientSide) {
            ...
            if (!event.isCancelled()) {
                this.aU = true;
                this.world.createExplosion(this, this.locXAtom.get(), this.locYAtom.get(), this.locZAtom.get(), event.getRadius(), event.getFire(), flag);
                this.die();
            } else {
                fuseTicks = 0;
            }
